...
class Environment(object):
    ...
    valid_inputs = {'light': TrafficLight.valid_states,
                    'oncoming': valid_actions, 'left': valid_actions, 'right': valid_actions}
    ...
    def __init__(self, verbose=False, num_dummies=100, grid_size=(8, 6)):
        ...
        self.trial_data = {
            ...
            # deadline value (time remaining) at the end
            'final_deadline': None,
    ...
    def create_agent(self, agent_class, *args, **kwargs):
        ...
        self.agent_states[agent] = {'location': random.choice(
            self.intersections.keys()), 'heading': (0, 1)}
        return agent
    ...
    def reset(self, testing=False, times=None):
        ...
        for agent in self.agent_states.iterkeys():
            ...
            # intersections and headings still in 'positions'
                ...
                if positions[intersection] == list():
                    # Delete the intersection altogether
                    del positions[intersection]

            agent.reset(destination=(
                destination if agent is self.primary_agent else None), testing=testing, times=times)
            if agent is self.primary_agent:
                # Reset metrics for this trial (step data will be set during
                # the step)
    ...
    def sense(self, agent):
        ...
        for other_agent, other_state in self.agent_states.iteritems():
            ...
            # This is because the primary agent is not required to follow the
            # waypoint
...
class Agent(object):
    ...
    def reset(self, destination=None, testing=False, times=None):
        pass
